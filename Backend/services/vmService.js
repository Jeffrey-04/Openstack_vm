...
function getPreferredAddress(server) {
  ...
  const preferredNetwork = networkNames.find((n) => /public|floating|ext|external/i.test(n));
  if (preferredNetwork && Array.isArray(addrs[preferredNetwork]) && addrs[preferredNetwork].length > 0) {
    const first = addrs[preferredNetwork].find((a) => a.version === 4 || a.addr);
...
async function enrichFlavorAndImage(server, vm, projectId) {
  ...
  if (flavorId) {
    try {
      ...
      // Keep existing flavor payload from Nova
    }
  }
...
async function buildServerView(vm, projectId) {
  try {
    ...
    if (raw.name && raw.name !== vm.name) {
      updates.name = raw.name;
    }
    if (Object.keys(updates).length) {
      vm.update(updates).catch(() => {});
    }
    const base = {
      ...raw,
      dbId: vm.id,
